...
const UPDATE_MATCH_PREDICTION_MUTATION = gql`
  mutation UpdateMatchPredictionMutation(
    $id: Int!
...
export const Success = ({
  matchPrediction,
}: CellSuccessProps<EditMatchPredictionById>) => {
  const [updateMatchPrediction, { loading, error }] = useMutation(
    UPDATE_MATCH_PREDICTION_MUTATION,
    {
      onCompleted: () => {
    ...
    <div className="rw-segment">
      <header className="rw-segment-header">
        <h2 className="rw-heading rw-heading-secondary">
          Edit MatchPrediction {matchPrediction.id}
        </h2>
      </header>
